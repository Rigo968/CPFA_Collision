...
void CPFA_qt_user_functions::DrawOnArena(CFloorEntity& entity) {
	...
	DrawPheromones();
	DrawNest();
	if(loopFunctions.DrawTrails == 1) DrawTrails();
...
void CPFA_qt_user_functions::DrawTrails() {
    ...
    for (const auto& robotDroppedTraj : d_t) {
        ...
        for (const auto& singleTraj : traj) {
            for (size_t j = 1; j < singleTraj.size(); ++j) {
                // Get the previous and current positions
                const auto& prev = singleTraj[j - 1];
                const auto& curr = singleTraj[j];
